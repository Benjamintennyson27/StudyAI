/* StudyAI Tauri backend. The frontend holds the app logic; the native side owns
...
const SERVICE: &str = "com.studyai.app";
...
fn youtube_extract(app: tauri::AppHandle, url: String) -> Result<YoutubeResult, String> {
    ...
    let dir = std::env::temp_dir().join(format!("studyai-yt-{}", std::process::id()));
