...
export function unsupportedMessage(task: Task): string {
  switch (task) {
    ...
    case "transcription":
      ...
      // setting anywhere to point StudyAI at a local Whisper server, so don't
      // suggest one. The one real path today is a cloud key.
      return "Local mode can't transcribe audio yet. Add an OpenAI key in Settings — StudyAI uses OpenAI's Whisper API automatically, nothing else to connect.";
    case "tts":
